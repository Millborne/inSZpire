import { Avatar, Button, ButtonsIcon, Dropdown, Modal, SnackbarAlert, TextContent } from "enterprisze-global-components";
import ConfirmationModal from "../ConfirmationModal";
import { useState } from "react";
import { Add, ArrowRight, MinusCirlce, TickCircle, Trash, Warning2 } from "iconsax-reactjs";

const TransferEmployeeModalSingleEmployee = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    const [onRemoveEmployeeIndex, setOnRemoveEmployeeIndex] = useState<Number | null>(null);
    ...
    const allEmployeesSaved = employeeTeamMember.every((member: any) => member.saved === true);
                ...
                content={
                    <div className="flex flex-col gap-2 mt-1">
                        <div className="w-full">
                            <span className="text-body-small-strong text-szDarkGrey600">You can only transfer employees if there are:</span>
                            <ul className="list-disc list-inside text-body-xsmall text-szDarkGrey600 pl-3 marker:text-[0.6em] marker:leading-[1.2]">
                                <li>
                                    <span className="text-body-small-strong text-szDarkGrey600">
                                        Available positions from other or its own team
                                    </span>
                                </li>
                                <li>
                                    <span className="text-body-small-strong text-szDarkGrey600">
                                        No Subordinates from that employee. Transfer the subordinate(s) first before transferring the
                                        employee.
                                    </span>
                                </li>
                            </ul>
                        ...
                        {employeeTeamMember.map((employee: any, index: number) =>
                            employee.saved ? (
                                        ...
                                        <div className="md:flex hidden w-full border border-error700 rounded-lg p-2 items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center justify-end gap-1">
                                                <Button
                                                    ...
                                                    variant="ghost"
                                                    size="medium"
                                                    onClick={() => setOnRemoveEmployeeIndex(null)}
                                                />
                                                {/* <Button
                                                  label="Confirm"
                                                  variant="ghost"
                                              ...
                                              /> */}
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        setEmployeeTeamMember(
                                                            employeeTeamMember.filter((member: any, i: number) => i !== index)
                                                        );
                                                ...
                                                >
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <span className="font-dmSans text-body-base-strong leading-none m-0">Confirm</span>
                                                    </div>
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            <div className="flex gap-5 items-center">
                                                <div className="flex flex-col gap-1">
                                                    <TextContent
                                                        icon={<Avatar src="/src/assets/noAvatar.png" size="small" />}
                                                    ...
                                                    />
                                                    <TextContent header={"previous team"} text={"No Previous Team"} />
                                                </div>
                                                <div>
                                                    <ArrowRight className="text-szPrimary900" />
                                                </div>
                                                <div className="flex flex-col gap-1">
                                                    <TextContent header={"Team"} text={employee.team_ID || "No Team Selected"} />
                                                    <TextContent
                                                        header={"position"}
                                                        text={employee.position_name || "No Position Selected"}
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <MinusCirlce
                                                    ...
                                                    onClick={() => setOnRemoveEmployeeIndex(index)}
                                                />
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="w-full border border-szGrey300 rounded-lg p-2 flex flex-col gap-2">
                                    <span className="text-caption-strong text-[#1D973C]">Transferring employee to available position</span>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <Dropdown
                                            ...
                                            onSelectionChange={(value: any) => {
                                                ...
                                                setEmployeeTeamMember(
                                                    employeeTeamMember.map((member: any, i: number) =>
                                                        i === index
                                                            ? {
                                                                  ...member,
                                                                  employee_first_name: value?.value?.split(", ")[1] || "",
                                                                  employee_last_name: value?.value?.split(", ")[0] || "",
                                                                  employee_ID: value?.value || "", // Using value as ID for now
                                                              }
                                                            : member
                                                    )
                                                );
                              ...
                              <p className="text-caption-reg text-red-500 mt-1">
                                  {errors.status}
                                    ...
                                    <span className="text-caption-reg text-szDarkGrey600">Select Team and Position</span>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <Dropdown
                                            label=""
                                            size="small"
                                                ...
                                                setEmployeeTeamMember(
                                                    employeeTeamMember.map((member: any, i: number) =>
                                                        i === index
                                                            ? {
                                                                  ...member,
                                                                  team_ID: value?.value || "", // Using value as team_ID for now
                                                              }
                                                            : member
                                                    )
                                                );
                                                ...
                                                setEmployeeTeamMember(
                                                    employeeTeamMember.map((member: any, i: number) =>
                                                        i === index
                                                            ? {
                                                                  ...member,
                                                                  position_ID: value?.value || "", // Using value as position_ID for now
                                                                  position_name: value?.value || "",
                                                              }
                                                            : member
                                                    )
                                                );
                                    ...
                                    <div className="flex justify-end gap-5">
                                        <ButtonsIcon
                                            icon={<Trash />}
                                            onClick={() => {
                                                setEmployeeTeamMember(employeeTeamMember.filter((member: any, i: number) => i !== index));
                                        ...
                                        <ButtonsIcon
                                            ...
                                            disabled={!selectedEmployee || !selectedTeam || !selectedPosition}
                                            variant="primary"
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="min-w-[180px]">
                                <Button
                                    ...
                                    size="small"
                                    leftIcon={<Add />}
                                    disabled={employeeTeamMember.some((member: any) => member.saved === false)}
                                />
                            </div>
