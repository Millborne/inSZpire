    ...
    const handleSavePosition = async (data: PositionDataType) => {
        try {
            if (modalMode === "add") {
                ...
                let resultArchive = await positionService.updatePosition(
                    positionData
                );
                ...
                if (resultUpdate.data?.success) {
                    ...
                    if (resultUpdate.error) {
                        if (
                            "data" in resultUpdate.error &&
                            resultUpdate.error.data
                        ) {
                ...
                if (data.is_archived === 1) {
                    ...
                    if (resultArchive.data?.success) {
                        ...
                        // Handle different error response formats from API
                        let errorMessage =
                            "An error occurred while archiving the position";

                        if (resultArchive.error) {
                            if (
                                "data" in resultArchive.error &&
                                resultArchive.error.data
                            ) {
                                const errorData = resultArchive.error
                                    .data as any;
                                if (errorData.message) {
                                    errorMessage = errorData.message;
                                } else if (
                                ...
                                errorMessage = (resultArchive.error as any)
                                    .message;
                            }
                        }
    ...
    const handleRestoreArchivePosition = async (index: number) => {
        ...
        try {
            if (position?.id) {
                ...
                let resultRestore = await positionService.updatePosition(
                    positionData
                );
                ...
                if (resultRestore.data?.success) {
                    ...
                    // Handle different error response formats from API
                    let errorMessage =
                        "An error occurred while restoring the position";

                    if (resultRestore.error) {
                        if (
                            "data" in resultRestore.error &&
                            resultRestore.error.data
                        ) {
    ...
    const handleArchiveConfirm = async () => {
        try {
            if (positionToArchive?.id) {
                const positionData = {
                    position_ID:
                        sanitizeUUID(positionToArchive.id) ||
                        positionToArchive.id,
                    is_archived: 1,
                    updated_by:
                        sanitizeUUID("567890abcdef1234567890abcdef1234") ||
                        "567890abcdef1234567890abcdef1234",
