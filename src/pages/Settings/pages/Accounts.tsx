...
const Accounts: React.FC<AccountsPageProps> = ({ mode }) => {
  ...
  const handleOpenModal = (account: AccountDataType, modalMode: ModalMode) => {
    setSelectedAccount({
      ...account,
      is_archived: mode === "archived" ? 1 : 0,
    });
  ...
  const handleSaveAccount = async (data: AccountDataType) => {
    try {
      if (modalMode === "add") {
        ...
        let resultUpdate = await accountService.updateAccount(accountDataEdit);
        let resultArchive = await accountService.updateAccount(accountData);
  ...
  const handleRestotrArchiveAccount = async (index: number) => {
    ...
    try {
      if (account?.id) {
        ...
        let resultRestore = await accountService.updateAccount(accountData);
  ...
  useEffect(() => {
    if (accountService.actionIsError) {
      const errorMessage =
        accountService.actionError && "data" in accountService.actionError
          ? (accountService.actionError.data as any)?.message
          : (accountService.actionError as any)?.message || "An error occurred";
      setSnackbarMessage(errorMessage);
      setSnackbarType("error");
      setIsSnackbarOpen(true);
    }
