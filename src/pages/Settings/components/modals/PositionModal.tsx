...
// assets
import positionConfirmation from "../../../../assets/position-confirmation.png";
    ...
    const fetchPositionTypes = async () => {
      if (isOpen) {
        ...
        try {
          ...
          const resultGetTotal = await positionTypeService.listPositionTypes(
            filtersGetTotal
          );
          ...
          if (resultGetTotal.data) {
            ...
            const result = await positionTypeService.listPositionTypes(filters);
    ...
    const fetchTags = async () => {
      if (isOpen) {
        ...
        try {
          ...
          const resultGetTotal = await tagService.viewTags(filtersGetTotal);
    ...
    const fetchWorkSetups = async () => {
      if (isOpen) {
        ...
        try {
          ...
          const resultGetTotal = await workSetupService.listWorkSetups(
            filtersGetTotal
          );
          if (resultGetTotal.data) {
            const filters = {
            ...
            const result = await workSetupService.listWorkSetups(filters);
    ...
    const fetchTeams = async () => {
      if (isOpen) {
        ...
        try {
          ...
          const resultGetTotal = await teamService.viewTeams(filtersGetTotal);
          if (resultGetTotal.data) {
            const filters = {
              offset: 0,
  ...
  useEffect(() => {
    if (
      ...
      let updatedFormData = {
        ...selectedPosition,
        team:
          teamOptions.find((team) => team.label === selectedPosition.team_ID)
            ?.value || "",
        jobTitle:
        ...
        position_status_name:
          positionStatusOptions.find(
            (type) => type.label === selectedPosition.position_status_name
          )?.value || "",
        work_setup_ID: workSetupOptions.find(
          (setup) => setup.label === selectedPosition.work_setup_ID
        )?.value,
        tag_IDs: selectedPosition.tag_IDs
          ?.map((tag) => tagOptions.find((opt) => opt.label === tag)?.value)
          ?.filter((id): id is string => id !== undefined),

        position_type_ID:
          positionTypeOptions.find(
            (type) => type.label === selectedPosition.position_type_ID
          )?.value || "",
        reports_to_position_ID:
          reportsToOptions.find(
            (opt) =>
              opt.value === selectedPosition.reports_to_position_ID ||
              opt.label === selectedPosition.reports_to_position_ID
          )?.value || "",
      };
  ...
  const handleConfirmationOpen = (action: "update" | "add" | "archive") => {
    ...
    if (action === "update") {
      ...
      const hasChanges =
        ...
        formData.position_type_ID !== selectedPositionData?.position_type_ID ||
        formData.work_setup_ID !== selectedPositionData?.work_setup_ID ||
          ...
          <ConfirmationContent
            ...
            data={[
              ...
              },
              {
                ...
                value:
                  positionTypeOptions.find(
                    (opt) => opt.value === formData.position_type_ID
                  )?.label || "—",
              },
              {
                label: "WORK SETUP",
                value:
                  workSetupOptions.find(
                    (opt) => opt.value === formData.work_setup_ID
                  )?.label || "—",
              },
              ...(formData.site_ID
                ? [
                    {
                      ...
                      value:
                        siteOptions.find(
                          (opt) => opt.value === formData.site_ID
                        )?.label || "—",
                    },
                  ]
                : []),
                ...
                ? [
                    {
                      ...
                      value:
                        formData.tag_IDs
                          .map(
                            (tagId) =>
                              tagOptions.find((opt) => opt.value === tagId)
                                ?.label
                          )
                          .filter(Boolean)
                          .join(", ") || "—",
                ...
                ? [
                    {
                      ...
                      value:
                        teamLevelOptions.find(
                          (opt) =>
                            opt.value === formData.team_level?.toString() ||
                            opt.label === formData.team_level?.toString()
                        )?.label || "—",
                    },
                  ]
                : []),
                ...
                ? [
                    {
                      ...
                      value:
                        reportsToOptions.find(
                          (opt) =>
                            opt.value === formData.reports_to_position_ID ||
                            opt.label === formData.reports_to_position_ID
                        )?.label || "—",
                    },
                  ]
                : []),
                  ...
                  ? [
                      {
                        label: "Position Name",
                        value: "",
                        oldValue: selectedPositionData?.position || "—",
                  ...
                  ? [
                      {
                        ...
                        oldValue:
                          teamOptions.find(
                            (opt) =>
                              opt.value === selectedPositionData?.team_ID ||
                              opt.label === selectedPositionData?.team_ID
                          )?.label || "—",
                        newValue:
                          teamOptions.find(
                            (opt) =>
                              opt.value === formData.team ||
                              opt.label === formData.team
                          )?.label || "—",
                      },
                    ]
                  : []),
                  ...
                  ? [
                      {
                        ...
                        oldValue:
                          positionReferenceOptions.find(
                            (opt) =>
                              opt.value === selectedPositionData?.jobTitle ||
                              opt.label === selectedPositionData?.jobTitle
                          )?.label || "—",
                        newValue:
                          positionReferenceOptions.find(
                            (opt) =>
                  ...
                  ? [
                      {
                        label: "Position Code",
                        value: "",
                        oldValue: selectedPositionData?.positionCode || "—",
                        newValue: formData.positionCode || "—",
                      },
                    ]
                  : []),
                ...(formData.basicSalary !== selectedPositionData?.basicSalary
                  ? [
                      {
                        ...
                        newValue: formData.basicSalary?.toString() || "—",
                      },
                    ]
                  : []),
                        ...
                        oldValue:
                          workSetupOptions.find(
                            (opt) =>
                              opt.value ===
                                selectedPositionData?.work_setup_ID ||
                              opt.label === selectedPositionData?.work_setup_ID
                          )?.label || "—",
                        newValue:
                          workSetupOptions.find(
                            (opt) =>
                              opt.value === formData.work_setup_ID ||
                              opt.label === formData.work_setup_ID
                          )?.label || "—",
                      },
                    ]
                  : []),
                  ...
                  ? [
                      {
                        ...
                        oldValue:
                          siteOptions.find(
                            (opt) =>
                              opt.value === selectedPositionData?.site_ID ||
                              opt.label === selectedPositionData?.site_ID
                          )?.label || "—",
                        newValue:
                          siteOptions.find(
                            (opt) =>
                              opt.value === formData.site_ID ||
                              opt.label === formData.site_ID
                          )?.label || "—",
                      },
                    ]
                  : []),
                  ...
                  <Dropdown
                    ...
                    value={
                      formData.team
                        ? teamOptions.find(
                            (opt) =>
                              opt.value === formData.team_ID ||
                              opt.label === formData.team_ID
                          )
                        : undefined
                    }
                    onSelectionChange={(value) => {
                      const teamValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange("team", teamValue || "");
                    }}
                    disabled={mode === "view" || isLoadingTeams}
                    // onScroll={(e) => {
              ...
              <div className="flex flex-col gap-[24px]">
                <div className="z-[9999]">
                  <Dropdown
                    ...
                    onSelectionChange={(value) => {
                      const jobTitleValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange("jobTitle", jobTitleValue || "");
                    }}
                    disabled={mode === "view" || isLoadingJobTitles}
                    // onScroll={(e) => {
                    //     setAddTenJobTitles(
                    //         addTenJobTitles + 10
                    //     );
              ...
              <div className="flex flex-col gap-[24px]">
                ...
                <div className="z-30">
                  <Dropdown
                    ...
                    disabled={mode === "view"}
                    onSelectionChange={(value) => {
                      const siteValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange("site_ID", siteValue || "");
                    }}
                    value={
                      formData.site_ID
                        ? siteOptions.find(
              ...
              <div className="flex flex-col gap-[24px]">
                <div className="z-[99]">
                  <Dropdown
                    ...
                    disabled={mode === "view" || isLoadingPositionTypes}
                    value={
                      formData.position_type_ID
                        ? positionTypeOptions.find(
                            (opt) =>
                              opt.value === formData.position_type_ID ||
                              opt.label === formData.position_type_ID
                          )
                        : undefined
                    }
                    onSelectionChange={(value) => {
                      const positionTypeValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange(
                        "position_type_ID",
                ...
                <div className="z-[60]">
                  <Dropdown
                    ...
                    disabled={mode === "view" || isLoadingWorkSetups}
                    // onScroll={(e) => {
                    //     setAddTenWorkSetups(
                    //         addTenWorkSetups + 10
                    //     );
                    ...
                    onSelectionChange={(value) => {
                      const workSetupValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange("work_setup_ID", workSetupValue || "");
                    }}
                  />
              ...
              <div className="flex flex-col gap-[24px]">
                ...
                <div className="z-[40]">
                  <Dropdown
                    ...
                    disabled={mode === "view"}
                    onSelectionChange={(value) => {
                      const reportsToValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange(
                        "reports_to_position_ID",
                ...
                <div className="z-[30]">
                  <Dropdown
                    ...
                    disabled={mode === "view"}
                    onSelectionChange={(value) => {
                      const teamLevelValue = Array.isArray(value)
                        ? value[0]?.value
                        : value?.value;
                      handleInputChange("team_level", teamLevelValue || "");
                    }}
                    value={
                      formData.team_level
                        ? teamLevelOptions.find(
              ...
              <div className="flex flex-col gap-[24px]">
                <div className="z-50">
                  <Dropdown
                    ...
                    multiSelect
                    isCheckbox
                    disabled={mode === "view" || isLoadingTags}
                    onSelectionChange={(value) => {
                      const tagsValue = Array.isArray(value)
                        ...
                        ? [value.value]
                        : [];
                      handleInputChange("tag_IDs", tagsValue);
                    }}
                    value={
                      formData.tag_IDs
                        ? formData.tag_IDs
                            ?.map((tag) =>
                              tagOptions.find(
                                (opt) => opt.label === tag || opt.value === tag
                              )
                            )
                            .filter(
